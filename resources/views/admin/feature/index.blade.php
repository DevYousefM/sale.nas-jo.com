        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="card">
              <div class="card-header border-bottom">
                <h5 class="card-title">{{ __('admin.features') }}</h5>
                    ...
                    @if(count($features) > 0)
                        @foreach ($features as $index=>$item )
                            <tr>
                                ...
                                @endforeach -->
                                <td >
                                    @if (!is_null($item->inputType))
                                        {{ __("admin.$item->inputType") }}
                                    @endif

                                </td>
                                <td>

                                    @if (Auth::guard('admin')->user()->hasPermission('feature-update'))
                                        <a class="btn btn-sm btn-icon" href="{{ route("feature.edit",$item->id) }}"><i class="bx bx-edit"></i></a>
                                    @endif

                                    @if (Auth::guard('admin')->user()->hasPermission('feature-delete'))
